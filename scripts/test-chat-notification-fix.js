...
async function testChatNotificationFix() {
  ...
  try {
    ...
    console.log('- Andriana sends message in a chat where she appears as both participants');
    ...
    console.log('Testing the additional safety checks in the notification service:');
    ...
    if (currentUserId === senderId) {
      console.log('✅ Check 2: Would skip (current user is sender) - NEW SAFETY CHECK');
    } else {
    ...
    if (targetUserId === currentUserId) {
      console.log('✅ Check 3: Would skip (target is current user) - NEW SAFETY CHECK');
    } else {
      console.log('✅ Check 3: Passed (target is not current user)');
    }
    
    console.log('\n🎯 Summary of fixes:');
    console.log('1. ✅ Removed problematic user context switching in addNotification');
    console.log('2. ✅ Added safety check: currentUserId === senderId');
    console.log('3. ✅ Added safety check: targetUserId === currentUserId');
    console.log('4. ✅ Notifications now only go to Supabase (realtime) instead of local state');
