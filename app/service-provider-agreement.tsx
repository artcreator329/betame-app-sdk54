...
export default function ServiceProviderAgreementScreen() {
  ...
  const handleAcceptAgreement = async () => {
    if (!user) {
      Alert.alert('Error', 'You must be signed in to accept the agreement.');
      return;
    }

    if (!hasAccepted) {
      Alert.alert('Agreement Required', 'Please confirm that you have read and accept the Service Provider Agreement.');
      return;
    }

    setIsAccepting(true);
    try {
      // Store the agreement acceptance in the database
      ...
      if (error) {
        ...
        if (error.code === '23505') { // Unique constraint violation
          ...
          if (updateError) {
            ...
            throw new Error('Failed to update agreement acceptance');
          }
        } else {
          console.error('Error storing agreement acceptance:', error);
          throw new Error('Failed to store agreement acceptance');
        }
      }

      Alert.alert(
        'Agreement Accepted',
        'Thank you for accepting the Service Provider Agreement. You can now proceed to provide your banking information.',
        [
          {
            text: 'Continue',
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background.primary }]}>
      {/* Header */}
      <View style={[styles.header, { backgroundColor: colors.background.primary, borderBottomColor: colors.border.main }]}>
        <TouchableOpacity
      ...
      >
        <View style={[styles.documentCard, { backgroundColor: colors.background.secondary, borderColor: colors.border.main }]}>
          <View style={styles.documentHeader}>
            <FileText size={24} color={colors.primary.main} />
            <Text style={[styles.documentTitle, { color: colors.text.primary }]}>
              BetaMe Service Provider Agreement
            </Text>
          </View>
          
          <Text style={[styles.documentSubtitle, { color: colors.text.secondary }]}>
            Please read this agreement carefully before proceeding
          </Text>
        </View>

        {/* Agreement Text */}
        <View style={[styles.agreementContent, { backgroundColor: colors.background.secondary, borderColor: colors.border.main }]}>
...
1. Scope
This Agreement governs the relationship between Providers and the Platform. Providers may offer both digital and physical services to customers ("Customers") through the Platform. The Platform serves solely as a marketplace to connect Customers with Providers and does not participate in the provision, supervision, or completion of services.
...
By registering as a Provider, you acknowledge that you have read, understood, and agreed to be bound by this Service Provider Agreement.`}
        ...
        >
          <View style={[
            ...
            hasAccepted && { backgroundColor: colors.primary.main, borderColor: colors.primary.main }
          ]}>
            {hasAccepted && <CheckCircle size={16} color="white" />}
          </View>
          <Text style={[styles.checkboxText, { color: colors.text.primary }]}>
            I have read, understood, and agree to be bound by this Service Provider Agreement
          </Text>
        </TouchableOpacity>
        ...
        <TouchableOpacity
          style={[
            styles.acceptButton,
            { backgroundColor: hasAccepted ? colors.primary.main : colors.border.main },
        ...
        >
          <Text style={[
            ...
            { color: hasAccepted ? 'white' : colors.text.tertiary }
          ]}>
