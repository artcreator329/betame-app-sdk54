...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    // For admin-only deployment, skip authentication check initially
    if (process.env.EXPO_PUBLIC_ADMIN_ONLY === 'true') {
      setIsAdmin(true);
      loadDashboardData();
    } else {
      checkAdminAccess();
  ...
  const loadDashboardData = async () => {
    try {
      setIsLoading(true);
      // For admin-only deployment, use mock data if service fails
      if (process.env.EXPO_PUBLIC_ADMIN_ONLY === 'true') {
        try {
